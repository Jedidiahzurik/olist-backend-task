...
router.get('/order_items', validateAcc, async (req, res) => {
...
router.delete('/order_items/:id', validateAcc, async (req, res) => {
	MongoClient.connect(
		mongoCredentials.url,
		mongoCredentials.options,
		async (err, client) => {
...
router.patch('/account', validateAcc, async (req, res) => {
	...
	MongoClient.connect(
		...
		async (err, client) => {
			...
			if (body.state && body.city) {
				data = { $set: { seller_city: body.city, seller_state: body.state } };
			} else if (body.state) {
				...
				return res
					.status(400)
					.json({ msg: 'user can only update city and state values' });
			}
